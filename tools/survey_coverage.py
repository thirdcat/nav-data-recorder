...
def voxel_view_counts(session_dir: str, *, voxel: float = 0.05, conf_min: int = 2,
    ...
    pairs = np.unique(np.stack([key, np.concatenate(bins)], axis=1), axis=0)
...
def format_table(results: list[dict[str, Any]]) -> str:
    head = (f"{'session':>8} {'frames':>6} {'voxels':>7} {'median':>6} "
            f"{'>=2':>6} {'>=3':>6} {'>=5':>6} {'range':>7} {'spread':>8} {'lin':>5}")
    lines = [head, "-" * len(head)]
    for r in results:
        if "error" in r:
            lines.append(f"{r['id'][-6:]:>8}  {r['error']}")
            continue
        f = r["frac_at_least"]
        lines.append(
            f"{r['id'][-6:]:>8} {r['frames']:>6} {r['surface_voxels']:>7} "
